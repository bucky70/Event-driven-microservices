...
public class KafkaAdminClient {
    ...
    public void createTopics(){
        ...
        try{
            ...
            throw new KafkaClientException("maximum number of retries for creating topics has been reached ",t);
        }
        checkTopicsCreated();
    }
    ...
    private void checkMaxRetry(int retry, Integer maxRetry) {
        if(retry>maxRetry){
            throw new KafkaClientException("reached max retry for kafka topics");
        }
    }
    ...
    private CreateTopicsResult doCreateTopics(RetryContext retryContext){ //loading list of string topic names and creating new topics with string values and multiplication factor and partitions from configdata
        ...
        List<NewTopic> kafkaTopics = topicNames.stream().map(topic ->
                new NewTopic(topic.trim(), kafkaConfigData.getNumOfPartitions(),
    ...
    private Collection<TopicListing> doGetTopics(RetryContext retryContext) throws ExecutionException, InterruptedException {
        LOG.info("reading {} topics(s), attempt {}", kafkaConfigData.getTopicNameToCreate().toArray(), retryContext.getRetryCount());
